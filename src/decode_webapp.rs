...
pub async fn happ_bytes_with_custom_properties(
    ...
    let app_bundle_bytes = modified_app_bundle.into_inner().pack().map_err(|e| {
        napi::Error::from_reason(format!(
...
pub async fn save_happ_or_webhapp(
    ...
    match WebAppBundle::unpack(happ_or_webhapp_bytes.as_slice()) {
        Ok(web_app_bundle) => {
            ...
            let ui_hash = hash_bytes_sha256(web_ui_zip_bytes.clone().into());
            ...
            let app_bundle_bytes = app_bundle.into_inner().pack().map_err(|e| {
                napi::Error::from_reason(format!("Failed to encode happ to bytes: {}", e))
            })?;

            let happ_hash = hash_bytes_sha256(app_bundle_bytes.clone().into());
            ...
            if let Some(dir) = uis_dir {
                ...
                // unzip and store UI
                fs::write(ui_zip_path.clone(), &web_ui_zip_bytes).map_err(|e| {
            ...
            fs::write(&happ_path, app_bundle_bytes).map_err(|e| {
        ...
        Err(_) => {
            let app_bundle = AppBundle::unpack(happ_or_webhapp_bytes.as_slice()).map_err(|e| {
...
pub async fn validate_happ_or_webhapp(
    ...
    let (app_bundle, maybe_ui_and_webhapp_hash) =
        match WebAppBundle::unpack(happ_or_webhapp_bytes.as_slice()) {
            Ok(web_app_bundle) => {
                ...
                let mut hasher = Sha256::new();
                hasher.update(web_ui_zip_bytes.clone());
            ...
            Err(_) => {
                let app_bundle =
                    AppBundle::unpack(happ_or_webhapp_bytes.as_slice()).map_err(|e| {
                        napi::Error::from_reason(format!("Failed to decode happ file: {}", e))
                    })?;
                (app_bundle, None)
            }
